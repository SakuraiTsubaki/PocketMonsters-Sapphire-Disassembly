...
- The following affine-frame/public-animation API block, `ApplyAffineAnimFrameRelativeAndUpdateMatrix` through `SetOamMatrixRotationScaling`, is mapped as 17 functions across all nine targets with **153 per-function SHA-256 fingerprints**.
- In the affine API block, 10 of 17 functions are raw-byte identical across all nine ROMs; 14 of 17 are raw-byte identical across the eight international ROMs.
- `EUR-AXPE-v1` = `USA-EUR-AXPE-v2`, French Rev 0 = Rev 1, and Italian Rev 0 = Rev 1 remain byte-identical throughout the completed animation and affine passes.
- Japanese `sprite.c` code uses an independent boundary map at AXPE `-0xE4` through the completed affine API block; DE/FR/IT retain international function sizes at AXPE `+0x134`.

See [`docs/STARTUP_ANALYSIS.md`](docs/STARTUP_ANALYSIS.md), [`docs/AGBMAIN_ANALYSIS.md`](docs/AGBMAIN_ANALYSIS.md), [`docs/MAIN_EARLY_FUNCTIONS.md`](docs/MAIN_EARLY_FUNCTIONS.md), [`docs/SPRITE_EARLY_FUNCTIONS.md`](docs/SPRITE_EARLY_FUNCTIONS.md), [`docs/SPRITE_RESOURCE_FUNCTIONS.md`](docs/SPRITE_RESOURCE_FUNCTIONS.md), [`docs/SPRITE_ANIMATION_FUNCTIONS.md`](docs/SPRITE_ANIMATION_FUNCTIONS.md), [`docs/SPRITE_AFFINE_CORE_FUNCTIONS.md`](docs/SPRITE_AFFINE_CORE_FUNCTIONS.md), [`docs/SPRITE_AFFINE_API_FUNCTIONS.md`](docs/SPRITE_AFFINE_API_FUNCTIONS.md), and [`docs/RECONSTRUCTION.md`](docs/RECONSTRUCTION.md).

The active next boundary is `LoadSpriteSheet`: `0x080021C4` JP, `0x080022A8` AXPE, and `0x080023DC` DE/FR/IT. The next pass continues into sprite-sheet loading, tile-range allocation, and tag lookup/freeing functions.
